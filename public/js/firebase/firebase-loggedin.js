					...
					$.each( feeds_object, function( key, item ) {
						...
						if(type == 'list') {
							...
							firebase.database().ref().child('Users/'+user.uid).on('value', snap => {
								user_feeds[message]['UserInitials'] = getInitials(snap.val().name);
							});
